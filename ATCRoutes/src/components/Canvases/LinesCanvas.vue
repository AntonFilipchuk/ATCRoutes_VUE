...
const watchedRoutesVisualProps = computed(() =>
  canvasStore.value.inactiveRoutes.map((route) => {
    return {
      ...route.routeVisuals.lineVisuals,
      ifShowLines: route.routeVisuals.ifShowLines,
...
function renderCanvas() {
  try {
    ...
    const routes = canvasStore.value.inactiveRoutes.filter(
      (route) => route.routeVisuals.ifVisible && route.routeVisuals.ifShowLines,
    )
