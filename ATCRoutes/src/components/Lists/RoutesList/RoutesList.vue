      ...
      <tbody>
        <tr v-if="activeRouteWithVisuals">
          <th>
            ...
            <button @click="toJson(activeRouteWithVisuals)">To JSON</button>
          </th>
          <td>
          ...
          <td>
            <TextVisualParameters
              v-model:visual="activeRouteWithVisuals.routeVisuals.textVisuals"
            />
          </td>
        </tr>
...
function toJson(route: CanvasRoute) {
  const points = route.route.getPoints().map((point) => {
    return {
      name: point.name,
      z: point.z,
      cartesianData: point.cartesianData,
      normalizedCartesianData: point.normalizedCartesianData,
      geographicCoordinate: point.geographicCoordinate,
    }
  })
  ...
  const toJson = JSON.stringify({
    name: route.name,
    points: points,
    routeVisuals: route.routeVisuals,
  })
