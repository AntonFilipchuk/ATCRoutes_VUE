<template>
  <div>
    <table>
      ...
      <tbody>
        <tr v-for="(intersection, index) in intersections" :key="index">
          <th>{{ index + 1 }}</th>
          <td>{{ intersection.route1.name }} - {{ intersection.route2.name }}</td>
          <td>
            [{{ intersection.sectionRoute1[0].name }} - {{ intersection.sectionRoute1[1].name }}] -
            [{{ intersection.sectionRoute2[0].name }} - {{ intersection.sectionRoute2[1].name }}]
          </td>
